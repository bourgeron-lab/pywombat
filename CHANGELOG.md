...
- **MNV module import error**: Fixed `'NoneType' object has no attribute 'VCF'` crash when `cyvcf2` is not installed, caused by runtime evaluation of type annotations referencing optional dependencies

## [1.5.2] - 2026-02-11

### Added

- **MHC region exclusion** (`mnv.exclude_mhc`): New YAML option to exclude variants in the MHC / HLA region before MNV detection
  - The MHC region on chromosome 6 has extremely high variant density and complex haplotype structure, leading to unreliable variant calling and many spurious MNV candidates
  - Configurable coordinates with GRCh38 xMHC defaults (chr6:28,510,120-33,480,577)
  - Matches both `chr6` and `6` chromosome naming conventions

### Changed

- **Renamed `examples/rare_coding.yml` to `examples/rare_coding_mnvs.yml`**: Updated example config with MNV detection, VEP annotation, MNV-only output, and MHC exclusion enabled

### Testing

- Added 5 new tests for MHC exclusion in `tests/test_filter.py`
